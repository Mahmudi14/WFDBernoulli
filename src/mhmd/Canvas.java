...
public class Canvas extends JPanel {
    ...
    public void paint(Graphics g) {
        ...
        if (water != null) {
            ...
            //Gambar Aliran
            double[][] dataKecepatan = water.getDataKecepatan();
            double[] batasKecepatan = water.getBatasKecepatan();
            Color[] warnaAliran = water.getWarnaKecepatan();
            int[][] result = water.getResult();

            if (showWater && dataKecepatan != null && warnaAliran != null && batasKecepatan != null && warnaAliran.length == batasKecepatan.length) {
                int rows = dataKecepatan.length;
                int cols = dataKecepatan[0].length;
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        double value = dataKecepatan[i][j];
                        //pilih warna base on value
                        Color c = Color.WHITE;
                        for (int k = 0; k < batasKecepatan.length; k++) {
                            c = warnaAliran[k];
                            if (value <= batasKecepatan[k]) {
                                break;
                            }
                        }
                        if (result[i][j] >= 0) {
